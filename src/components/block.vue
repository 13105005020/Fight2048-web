        ...
        <div>
            ...
            <div class="box">
                <div class="row" v-for="(row,index) in list" :key="index">
                    <div class="col" :class="'n-'+col.type" v-for="(col,key) in row" :key="key">{{col.number}}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    export default {
        ...
        methods: {
            ...
            newGame() {
                ...
                this.score = 0
                this.over = false
                this.list = Array.from(Array(this.size)).map(() => Array(this.size).fill({number:undefined,type:0}))
                this.setRandom()
            },
            //插入新格子
            setRandom() {
                        ...
                        if (cell) {
                            for (let dir = 0; dir < 4; dir++) {
                                ...
                                if (this.withinBounds(i + vector.x, j + vector.y)) {
                                    let other = this.list[i + vector.x][j + vector.y].number
                                    if (other && other === cell.number) {
                                        return true
                                    }
                                }
                            }
            ...
            moveLeft(list) {
                ...
                _list.forEach(item => {
                    ...
                    if (next && next.number != undefined){
                        number = next.number * 2
                    }
                    if (item.value.type !== this.round){
                        return
                    }
                    if (next&&next.number && next.number === item.value.number&&next.type===item.value.type && !_list[farthest].merged) {
                        //合并
                        list[item.x] = {number:undefined,type:0}
                        list[farthest-1] = {number:number,type:item.value.type}
                        item = {
                            x: farthest-1 ,
                            merged: true,
                            value: {number:number,type:item.value.type}
                        }
                        if (number!=undefined){
                           this.score += number
                        }
                        ...
                        if (farthest != item.x) {
                            list[farthest] ={number:item.value.number,type:item.value.type} 
                            list[item.x] = {number:undefined,type:0}
                            item.x = farthest
                        }
                    }
            ...
            rotate(arr, n) {
                n = n % 4
                if (n === 0) return arr
                let tmp = Array.from(Array(this.size)).map(() => Array(this.size).fill({number:undefined,type:0}))
                for (let i = 0; i < this.size; i++) {
                    for (let j = 0; j < this.size; j++) {
            ...
            farthestPosition(list, cell) {
                let farthest = cell.x
                while (farthest > 0 && !list[farthest - 1].number) {
                    farthest = farthest - 1
                }
                return farthest
            },
